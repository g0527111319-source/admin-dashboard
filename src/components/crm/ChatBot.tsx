...
type QAPair = {
  keywords: string[];
  synonyms?: string[];
  answer: string | ((ctx: DesignerContext, gender?: string) => string);
  relatedTopics?: string[];
...
function personalGreeting(ctx: DesignerContext, gender?: string): string {
  const name = firstName(ctx);
  if (!name) return "";
  const dear = g(gender, "יקר", "יקרה");
  return pick([`${name}, `, `היי ${name}! `, `${name} ${dear}, `]);
}
...
type ConversationalPattern = {
  patterns: RegExp[];
  responses: string[] | ((ctx: DesignerContext, gender?: string) => string[]);
  relatedTopics?: string[];
};
...
const conversationalPatterns: ConversationalPattern[] = [
  // Greetings
  {
    ...
    responses: (ctx, gender) => {
      const name = firstName(ctx);
      const timeG = getTimeGreeting();
      const dear = g(gender, "יקר", "יקרה");
      const happy = g(gender, "שמח", "שמחה");
      const askAnything = g(gender, "שאל כל דבר", "שאלי כל דבר");
      const whatToKnow = g(gender, "מה תרצה לדעת?", "מה תרצי לדעת?");
      return name ? [
        `${timeG} ${name}! 😊 ${happy} לראות אותך. איך אפשר לעזור היום?`,
        `היי ${name}! ✨ ${timeG}. ${whatToKnow}`,
        `${name} ${dear}, ${timeG}! 💛 אני כאן בשבילך. במה אוכל לעזור?`,
      ] : [
        `${timeG}! ✨ ${happy} ${g(gender, "שפנית", "שפנית")} אליי. איך אפשר לעזור?`,
        `היי! 😊 ${timeG}. אני כאן בשבילך — ${askAnything}!`,
      ];
    },
  ...
  // Thanks
  {
    ...
    responses: (ctx, gender) => {
      const name = firstName(ctx);
      return [
        `${name ? name + ", " : ""}בשמחה! 💛 אני תמיד כאן אם ${g(gender, "תצטרך", "תצטרכי")} עוד משהו.`,
        `${g(gender, "שמח", "שמחה")} שעזרתי! ✨ ${name ? `אל ${g(gender, "תהסס", "תהססי")} לפנות שוב, ` + name : `${g(gender, "פנה", "פני")} אליי בכל עת`}.`,
  ...
  // Frustration / help
  {
    patterns: [/^(לא מבינה|לא מבין|לא מצליחה|לא מצליח|עוזרי|עזרה|אני צריכה|אני צריך|בעיה|תקועה|תקוע|לא עובד|באג|שגיאה|נתקעתי)/],
    responses: (ctx, gender) => {
      const name = firstName(ctx);
      return [
        `${name ? name + ", " : ""}אני כאן בדיוק בשביל זה! 🤗 ${g(gender, "ספר", "ספרי")} לי מה קורה ואני אעזור צעד אחרי צעד.`,
        `לא נורא! 💪 ${name ? name + ", " : ""}${g(gender, "בוא", "בואי")} נפתור את זה ביחד. על איזה נושא מדובר?`,
        `${name ? name + ", " : ""}אל דאגה, הכל בסדר. 😊 ${g(gender, "תאר", "תתארי")} לי את הבעיה ואני אדריך אותך.`,
      ];
    },
  ...
  // Who are you
  {
    patterns: [/^(מי את|מה את|מי אתה|מה אתה|ספרי על עצמך|ספר על עצמך|מה את יודעת|מה אתה יודע|מה את עושה|מה אתה עושה)/],
    responses: (ctx, gender) => {
      const name = firstName(ctx);
      const helper = g(gender, "העוזר החכם", "העוזרת החכמה");
      return [
        `${name ? name + ", " : ""}אני ${helper} של זירת האדריכלות! 🤖✨\n\nאני ${g(gender, "מכיר", "מכירה")} כל פינה במערכת — מניהול לקוחות ופרויקטים, דרך חוזים דיגיטליים, ועד מעקב תקציב ותזמון.\n\nאני כאן 24/7 ותמיד ${g(gender, "שמח", "שמחה")} לעזור. ${g(gender, "שאל", "שאלי")} כל דבר!`,
      ];
    },
    relatedTopics: ["מה אפשר לעשות?", "ניווט באתר"],
  },
  // Compliment
  {
    ...
    responses: (ctx, gender) => {
      const name = firstName(ctx);
      return [
        `תודה${name ? " " + name : ""}! 🥰 ${g(gender, "שמח שאתה מרוצה", "שמחה שאת מרוצה")}. יש עוד הרבה דברים מגניבים במערכת — ${g(gender, "רוצה", "רוצה")} לגלות?`,
        `איזה כיף! ✨ ${name ? name + ", " : ""}המערכת באמת עמוסה ביכולות. ${g(gender, "רוצה", "רוצה")} שאראה לך עוד?`,
      ];
    },
...
const knowledgeBase: QAPair[] = [
  {
    ...
    answer: (ctx, gender) => {
      const name = firstName(ctx);
      const dear = g(gender, "יקר", "יקרה");
      const come = g(gender, "בוא", "בואי");
      const intro = name
        ? pick([`${name}, ניהול לקוחות זה הלב`, `${name} ${dear}, ${come} נדבר על לקוחות`])
        : "ניהול לקוחות — הלב";
      return `${intro} של ה-CRM שלך! 💛\n\n📍 מיקום: תפריט צד ← CRM ← לקוחות\n\nאיך מתחילים?\n1️⃣ ${g(gender, "לחץ", "לחצי")} \"+ לקוח חדש\" בפינה העליונה\n2️⃣ ${g(gender, "מלא", "מלאי")} פרטים: שם מלא, טלפון, אימייל, כתובת\n3️⃣ הלקוח מופיע ברשימה — ומכאן הכל זורם!\n\n🔹 שיוך: כל לקוח ניתן לשייך לפרויקט אחד או יותר\n🔹 כרטיס לקוח: מרכז הכל — פרויקטים, חוזים, תשלומים, הערות\n🔹 חיפוש: ${g(gender, "סנן", "סנני")} לפי שם, סטטוס, או פרויקט\n🔹 פעולות מהירות: ישירות מהכרטיס — ${g(gender, "צור", "צרי")} פרויקט, ${g(gender, "שלח", "שלחי")} חוזה, ${g(gender, "פתח", "פתחי")} WhatsApp\n\n💡 טיפ מקצועי: ${g(gender, "הוסף", "הוסיפי")} הערות פנימיות לכל לקוח (רק ${g(gender, "אתה רואה", "את רואה")} אותן!) — לדוגמה, מה הלקוחה אמרה שחשוב לה, מה הסגנון שאהבה, תאריכי תשלום.`;
    },
  ...
  },
  {
    ...
    answer: (ctx, gender) => {
      const name = firstName(ctx);
      const dear = g(gender, "יקר", "יקרה");
      const prefix = name ? pick([`${name}, `, `${name} ${dear}, `]) : "";
      return `${prefix}מערכת החוזים הדיגיטליים היא אחד הכלים הכי חזקים שלך! 📝✨\n\nהתהליך המלא — 5 שלבים פשוטים:\n\n1️⃣ תבנית: חוזים ← \"תבניות\" ← \"+ תבנית חדשה\"\n   📎 ${g(gender, "העלה", "העלי")} PDF/תמונה של החוזה שלך, או ${g(gender, "בנה", "בני")} מאפס\n   ✏️ ${g(gender, "הוסף", "הוסיפי")} שדות: פרטי ${g(gender, "מעצב", "מעצבת")}, פרטי לקוח, חתימה\n\n2️⃣ חוזה חדש: ${g(gender, "בחר", "בחרי")} תבנית → ${g(gender, "מלא", "מלאי")} פרטים → טיוטה מוכנה\n\n3️⃣ ${g(gender, "חתום כמעצב", "חתמי כמעצבת")}: אייקון העט → ${g(gender, "חתום", "חתמי")} בקנבס עם האצבע\n\n4️⃣ ${g(gender, "שלח", "שלחי")} ללקוח: הלקוח מקבל קישור ייחודי ומאובטח\n\n5️⃣ הלקוח חותם → שניכם מקבלים אישור מיידי ✅\n\n📊 מעקב: 👁️ לקוח צפה / ✅ לקוח חתם / ⏳ ממתין\n\n💡 טיפ מנצח: ${g(gender, "העתק", "העתיקי")} את קישור החתימה ${g(gender, "ושלח", "ושלחי")} ישירות בוואטסאפ — שיעור החתימה גבוה פי 3 לעומת מייל!`;
    },
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `העלאת תבנית חוזה — פעם אחת, משתמשים לנצח! 📋\n\n📍 מיקום: חוזים ← לשונית "תבניות" ← "+ תבנית חדשה"\n\n🔹 העלאה: ${g(gender, "גרור", "גררי")} קובץ PDF, PNG, JPG או WEBP\n🔹 בנייה מאפס: ${g(gender, "הוסף", "הוסיפי")} בלוקים — כותרת, פסקה, מפריד, רווח\n🔹 שדות חכמים: שדות ${g(gender, "מעצב", "מעצבת")} (נמלאים אוטומטית!), שדות לקוח, שדות חתימה דיגיטלית\n🔹 ברירת מחדל: ${g(gender, "סמן", "סמני")} תבנית אחת כברירת מחדל — ${g(gender, "תחסוך", "תחסוך")} לך זמן\n\n💡 הטיפ שלי: ${g(gender, "קח", "קחי")} את החוזה הנוכחי שלך → ${g(gender, "העלה", "העלי")} כ-PDF → ${g(gender, "הוסף", "הוסיפי")} רק שדות חתימה → ${g(gender, "מוכן", "מוכנה")} תוך 2 דקות!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `שליחת חוזה לחתימה — פשוט כמו לשלוח הודעה! ✍️\n\n📍 מיקום: חוזים ← ${g(gender, "בחר", "בחרי")} חוזה בסטטוס "טיוטה"\n\n🔹 ${g(gender, "חתום", "חתמי")} קודם: ${g(gender, "לחץ", "לחצי")} על 🖊️ → ${g(gender, "חתום בקנבס כמעצב", "חתמי בקנבס כמעצבת")}\n🔹 ${g(gender, "שלח", "שלחי")}: ${g(gender, "לחץ", "לחצי")} ← הלקוח מקבל קישור ייחודי ומאובטח\n🔹 קישור ידני: ${g(gender, "לחץ", "לחצי")} על 🔗 → מעתיק קישור חתימה\n\n📊 מעקב בזמן אמת:\n   👁️ "לקוח צפה" — הלקוח פתח את הקישור\n   ✅ "לקוח חתם" — הושלם! שניכם מקבלים אישור\n\n🔒 אבטחה: כל חתימה מתועדת עם IP, תאריך ושעה\n\n💡 טיפ: לפני שליחה → ${g(gender, "לחץ", "לחצי")} "תצוגה מקדימה" לוודא שהכל מושלם.`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `צ'קליסט מסירה — אף פרט לא נשכח! ✅\n\n📍 מיקום: תפריט צד ← CRM ← צ'קליסט מסירה\n\nאיך זה עובד:\n1️⃣ ${g(gender, "בחר", "בחרי")} פרויקט ← נוצרים 11 פריטי ברירת מחדל\n2️⃣ ${g(gender, "סמן", "סמני")} V על כל פריט שהושלם ← ${g(gender, "הוסף", "הוסיפי")} הערה\n3️⃣ כשהכל ירוק — הפרויקט מוכן למסירה! 🎉\n\n📋 קטגוריות ברירת מחדל:\n   🍳 מטבח | 🚿 חדרים רטובים | 🛋️ סלון | 🛏️ חדר שינה | ⚙️ כללי\n\n🔹 כל פריט: אחראי (${g(gender, "מעצב", "מעצבת")} / לקוח) + סטטוס + הערות\n📊 מעקב: פס התקדמות צבעוני + אחוז השלמה\n\n💡 טיפ: ${g(gender, "הוסף", "הוסיפי")} פריטים ייחודיים לכל פרויקט — כי כל שיפוץ שונה!`,
  ...
  },
  {
    ...
    answer: (ctx, gender) => {
      const name = firstName(ctx);
      const prefix = name ? `${name}, ` : "";
      return `${prefix}מעקב תקציב — שליטה מלאה בכל שקל! 💰\n\n📍 מיקום: תפריט צד ← CRM ← מעקב תקציב\n\nאיך עובדים עם זה:\n1️⃣ ${g(gender, "בחר", "בחרי")} פרויקט → ${g(gender, "הגדר", "הגדירי")} תקציב כולל\n2️⃣ ${g(gender, "הוסף", "הוסיפי")} פריטים לפי קטגוריות: מטבח, ריצוף, תאורה, נגרות, אינסטלציה...\n3️⃣ לכל פריט: סכום מתוכנן ← סכום בפועל ← ספק ← סטטוס\n\n📊 דשבורד תקציב:\n   💵 סה\"כ מתוכנן vs. בפועל\n   📈 אחוז ניצול לכל קטגוריה\n   ⚠️ התראת חריגה אוטומטית\n   📊 גרף פילוח הוצאות\n\n🔹 סטטוסים: מתוכנן → הצעה התקבלה → מאושר → שולם\n\n💡 טיפ מקצועי: ${g(gender, "קשר", "קשרי")} כל פריט תקציב להצעת מחיר של ספק — ככה ${g(gender, "אתה רואה", "את רואה")} הכל ממקום אחד ואף חריגה לא מפתיעה!`;
    },
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `תזמון חכם — ציר זמן ויזואלי שמונע עיכובים! 📅\n\n📍 מיקום: תפריט צד ← CRM ← תזמון חכם\n\n🔹 תצוגת Gantt: ציר זמן אופקי מקצועי — כל שלב בצבע אחר\n🔹 יצירת שלב: שם ← תאריך התחלה ← משך בימים ← תלויות\n🔹 תלויות: ${g(gender, "הגדר", "הגדירי")} "שלב X מתחיל רק אחרי שלב Y" — המערכת מנהלת אוטומטית\n\n📊 סטטוסים צבעוניים:\n   ⬜ ממתין | 🔵 בתהליך | ✅ הושלם | 🔴 באיחור\n\n🔹 ספקים: ${g(gender, "שייך", "שייכי")} ספק + טלפון לכל שלב — ${g(gender, "תקשר", "תקשרי")} ישירות\n🔹 התראות: ⚠️ המערכת מתריעה כשתלות מתעכבת!\n\n💡 טיפ: שלב באיחור? ${g(gender, "שנה", "שני")} ל"באיחור" + ${g(gender, "תרשום", "תרשמי")} סיבה. ככה בסוף הפרויקט יש לך תיעוד מלא.`,
  ...
  },
  {
    ...
    category: "design",
    answer: (_ctx, gender) => `ספריית השראה — ה-Pinterest הפרטי שלך! 🎨\n\n📍 מיקום: תפריט צד ← CRM ← ספריית השראה\n\nאיך עובדים:\n1️⃣ ${g(gender, "צור", "צרי")} בורדים: לפי פרויקט, סגנון, או חדר\n2️⃣ ${g(gender, "הוסף", "הוסיפי")} תמונות: ${g(gender, "גרור", "גררי")} מהמחשב או ${g(gender, "הדבק", "הדביקי")} לינק\n3️⃣ ${g(gender, "תייג", "תייגי")}: סגנון (מודרני/סקנדינבי/קלאסי...) + חדר + חומר\n\n🔹 שיתוף חכם: ${g(gender, "הפעל", "הפעילי")} "שתף עם לקוח" → הלקוחה מסמנת ❤️ על מה שאהבה\n🔹 סינון: ${g(gender, "מצא", "מצאי")} תמונות לפי סגנון / חדר / חומר\n🔹 תצוגה: גריד יפה עם hover preview\n\n💡 טיפ: ${g(gender, "שלח", "שלחי")} קישור שיתוף בוואטסאפ → הלקוחה תסמן מה אהבה → ${g(gender, "אתה תראה", "את תראי")} בדיוק מה מתאים לה. חוסך שעות של פגישות!`,
  ...
  },
  {
    ...
    category: "financial",
    answer: (_ctx, gender) => `הצעות מחיר — מקצועי ומרשים! 💎\n\n📍 מיקום: תפריט צד ← CRM ← הצעות מחיר\n\nיצירת הצעה:\n1️⃣ "+ הצעה חדשה" → ${g(gender, "בחר", "בחרי")} לקוח ← פרויקט\n2️⃣ ${g(gender, "הוסף", "הוסיפי")} פריטים: תיאור, כמות, מחיר ליחידה\n3️⃣ סכום כולל — מחושב אוטומטית!\n\n📊 מחזור חיים:\n   📝 טיוטה → 📤 נשלחה → ✅ אושרה → ❌ נדחתה\n\n🔥 פיצ'ר מנצח: הצעה שאושרה → ${g(gender, "לחץ", "לחצי")} "המר לחוזה" → חוזה נוצר אוטומטית עם כל הפרטים!\n\n💡 טיפ: ${g(gender, "הגדר", "הגדירי")} תבנית הצעה בהגדרות CRM — ${g(gender, "חוסך", "חוסכת")} זמן בכל הצעה חדשה.`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `יומן ופגישות — הזמן שלך מנוהל! 📅\n\n📍 מיקום: תפריט צד ← CRM ← יומן\n\n🔹 תצוגות: חודש 📆 | שבוע 📋 | יום ⏰\n🔹 יצירה: ${g(gender, "לחץ", "לחצי")} על תאריך בלוח או "+ פגישה חדשה"\n🔹 פרטים: כותרת, תאריך ושעה, שיוך ללקוח, מיקום (כתובת / זום)\n🔹 תזכורות: ${g(gender, "קבע", "קבעי")} תזכורת 15 דק / שעה / יום לפני\n🔹 גרור-ושחרר: ${g(gender, "שנה", "שני")} תאריך פגישה בגרירה פשוטה!\n\n💡 טיפ מקצועי: ${g(gender, "הוסף", "הוסיפי")} הערה לכל פגישה עם מה לדבר — ${g(gender, "תגיע מוכן ותרשים", "תגיעי מוכנה ותרשימי")} את הלקוח!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `ניהול ספקים — שתי מערכות שעובדות ביחד! 🤝\n\n📍 ספריית הקהילה: תפריט צד ← קהילה ← ספריית ספקים\n   🌐 כל ספקי הקהילה: חיפוש לפי קטגוריה, אזור\n   ⭐ דירוגים והמלצות של ${g(gender, "מעצבים אחרים", "מעצבות אחרות")}\n   📊 היסטוריית עסקאות לכל ספק\n\n📍 הספקים שלי: תפריט צד ← עיצוב ← ספקים שלי\n   📝 ניהול אישי: שם, תחום, טלפון, הערות פרטיות\n   🔗 ${g(gender, "שייך", "שייכי")} ספקים לפרויקטים\n   📞 חיוג ישיר מהכרטיס\n\n💡 טיפ: ספק ${g(gender, "שדירגת", "שדירגת")} בספרייה → יופיע גבוה יותר ויעזור ${g(gender, "למעצבים אחרים", "למעצבות אחרות")}. קהילה חזקה!`,
  ...
  },
  {
    ...
    answer: (_ctx, gender) => `WhatsApp — תקשורת ישירה מהמערכת! 📱\n\n📍 מיקום: תפריט צד ← CRM ← WhatsApp\n\nחיבור ראשוני (פעם אחת):\n1️⃣ ${g(gender, "לחץ", "לחצי")} "חבר WhatsApp"\n2️⃣ ${g(gender, "סרוק", "סרקי")} QR code עם הוואטסאפ שלך\n3️⃣ מחובר! ✅ הודעות נשלחות ישירות\n\n🔹 שליחה: ${g(gender, "כתוב", "כתבי")} הודעה → ${g(gender, "שלח", "שלחי")} ישירות ללקוח\n🔹 תבניות: הודעות מוגדרות מראש — חוסכות זמן\n🔹 היסטוריה: כל ההתכתבות שמורה בכרטיס הלקוח\n\n🔥 שילובים חכמים:\n   📎 ${g(gender, "שלח", "שלחי")} קישור חתימת חוזה\n   📋 ${g(gender, "שלח", "שלחי")} סיכום פגישה\n   🔗 ${g(gender, "שלח", "שלחי")} קישור לבורד השראה\n\n💡 טיפ: ${g(gender, "הגדר", "הגדירי")} תבנית "הודעת פתיחה" — ברגע שלקוחה חדשה מתווספת, ${g(gender, "שלח", "שלחי")} מיד!`,
  ...
  },
  {
    ...
    category: "design",
    answer: (_ctx, gender) => `מודבורד — לוח עיצוב שהלקוח אוהב! 🎨\n\n📍 מיקום: תפריט צד ← CRM ← מודבורד\n\n🔹 יצירה: "+ מודבורד חדש" ← שם ← ${g(gender, "שייך", "שייכי")} לפרויקט\n🔹 ${g(gender, "הוסף", "הוסיפי")}: תמונות, טקסטים, דוגמאות צבע, חומרים, הערות\n🔹 סידור: ${g(gender, "גרור ושחרר", "גררי ושחררי")} ← ${g(gender, "סדר", "סדרי")} כרצונך\n🔹 שיתוף: ${g(gender, "שתף", "שתפי")} עם הלקוח — הוא רואה ומגיב!\n\n✨ ההבדל ממודבורד לספריית השראה:\n   📌 מודבורד = קונספט כללי, מצב רוח, כיוון\n   📸 ספריית השראה = תמונות ספציפיות, רפרנסים\n\n💡 טיפ: ${g(gender, "הכן", "הכיני")} מודבורד לכל חדר — הלקוח מרגיש ${g(gender, "שהבנת", "שהבנת")} אותו!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `ניהול חומרים — מעקב מדויק מהזמנה להתקנה! 📦\n\n📍 מיקום: תפריט צד ← CRM ← חומרים\n\nלכל חומר:\n🔹 שם, ספק, כמות, מחיר, תמונה\n🔹 שיוך לפרויקט ולחדר\n\n📊 סטטוסים:\n   📋 הוזמן → 🚚 בדרך → ✅ הגיע → 🔧 הותקן\n\n🔹 סיכום: עלויות מצטברות לפרויקט\n🔹 קישור חכם: ${g(gender, "קשר", "קשרי")} לפריט תקציב ← מעקב כפול אוטומטי\n\n💡 טיפ: כשחומר מגיע — ${g(gender, "עדכן", "עדכני")} מיד + ${g(gender, "צלם", "צלמי")}! ככה יש תיעוד אם משהו לא מתאים.`,
  ...
  },
  {
    ...
    category: "design",
    answer: (_ctx, gender) => `לפני/אחרי — ${g(gender, "הצג", "הציגי")} את הקסם ${g(gender, "שעשית", "שעשית")}! ✨\n\n📍 מיקום: תפריט צד ← CRM ← לפני/אחרי\n\n🔹 יצירה: ${g(gender, "בחר", "בחרי")} פרויקט ← ${g(gender, "העלה", "העלי")} תמונת "לפני" + "אחרי"\n🔹 סלייד: תצוגת סלייד מרשימה להשוואה ויזואלית\n🔹 שיתוף: ${g(gender, "הוסף", "הוסיפי")} לפרופיל ← לקוחות פוטנציאליים רואים\n\n📸 טיפים לצילום מנצח:\n   📐 ${g(gender, "צלם", "צלמי")} מאותה זווית בדיוק — אפקט "וואו" חזק!\n   💡 אותה תאורה — אל ${g(gender, "תצלם", "תצלמי")} ביום שונה אם אפשר\n   🧹 נקי ומסודר — בלפני ובאחרי\n\n💡 טיפ: לפני/אחרי טובים = כלי השיווק הכי חזק שלך!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `קליטת לקוח — הרושם הראשון חשוב! 🌟\n\n📍 מיקום: תפריט צד ← CRM ← קליטת לקוח\n\nאיך זה עובד:\n1️⃣ ${g(gender, "צור", "צרי")} טופס קליטה ← ${g(gender, "בחר", "בחרי")} שאלות\n2️⃣ ${g(gender, "שלח", "שלחי")} קישור ← הלקוח ממלא בנוחות\n3️⃣ התשובות נשמרות אוטומטית בכרטיס הלקוח\n\n📋 שאלות מומלצות:\n   🎨 סגנון מועדף + תמונות דוגמה\n   💰 טווח תקציב\n   📅 לוח זמנים רצוי\n   📐 רשימת חדרים / דרישות מיוחדות\n   👨‍👩‍👧‍👦 הרכב המשפחה + אורח חיים\n\n💡 טיפ מקצועי: ${g(gender, "שלב", "שלבי")} עם שאלון סגנון — ${g(gender, "תגיע", "תגיעי")} לפגישה הראשונה כבר עם תמונה מלאה של הלקוח!`,
  ...
  },
  {
    ...
    category: "design",
    answer: (_ctx, gender) => `שאלון סגנון — ${g(gender, "גלה", "גלי")} מה הלקוח באמת אוהב! 🎭\n\n📍 מיקום: תפריט צד ← CRM ← שאלון סגנון\n\n✨ איך זה עובד:\n🔹 שאלון ויזואלי: הלקוח בוחר תמונות שאהב\n🔹 ניתוח אוטומטי: המערכת מזהה סגנון דומיננטי\n🔹 תוצאה: דף תוצאות מרשים + המלצות\n\n🎨 סגנונות: מודרני, סקנדינבי, אינדוסטריאלי, קלאסי, בוהו, מינימליסטי, ים-תיכוני...\n\n💡 טיפ שמרשים: ${g(gender, "שתף", "שתפי")} את דף התוצאות בפגישה הראשונה — הלקוח מרגיש ${g(gender, "ש\"הבנת אותו\"", "ש\"הבנת אותו\"")} מהרגע הראשון! זה בונה אמון מהיום הראשון.`,
  ...
  },
  {
    ...
    category: "financial",
    answer: (_ctx, gender) => `מעקב שעות — ${g(gender, "דע", "דעי")} בדיוק כמה השקעת! ⏱️\n\n📍 מיקום: תפריט צד ← CRM ← מעקב שעות\n\n🔹 טיימר חכם: ▶️ ${g(gender, "התחל", "התחילי")} ← ⏸️ ${g(gender, "עצור", "עצרי")} ← נשמר אוטומטית\n🔹 הוספה ידנית: תאריך ← שעות ← תיאור ← פרויקט\n🔹 קטגוריות: ייעוץ, תכנון, ליווי, ביקור באתר, רכש\n\n📊 דוחות:\n   📋 סיכום לפרויקט — כמה שעות על כל פרויקט\n   📅 סיכום חודשי — כמה שעות עבדת החודש\n   👤 סיכום ללקוח — כמה שעות לכל לקוח\n\n💡 למה זה חשוב? אם ${g(gender, "אתה גובה", "את גובה")} לפי שעה — תיעוד מדויק. אם לא — ${g(gender, "תבדוק אם אתה מתמחר", "תבדקי אם את מתמחרת")} נכון! ככה ${g(gender, "תדע", "תדעי")} בפרויקט הבא כמה לגבות.`,
  ...
  },
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `הגדרות CRM — ${g(gender, "התאם", "התאימי")} את המערכת אליך! ⚙️\n\n📍 מיקום: תפריט צד ← CRM ← הגדרות CRM\n\n🔹 ברירות מחדל:\n   📊 שלבי פרויקט — ${g(gender, "הגדר", "הגדירי")} את השלבים שלך\n   💰 תנאי תשלום — ברירת מחדל לחוזים\n   📝 תבנית חוזה — ${g(gender, "בחר", "בחרי")} תבנית ברירת מחדל\n   📋 תבנית הצעה — מתאימה מראש\n\n🔹 כלליות:\n   💱 מטבע (₪ / $ / €)\n   🕐 אזור זמן\n   🌐 שפת הודעות ללקוח\n\n💡 טיפ: ${g(gender, "הגדר", "הגדירי")} פעם אחת — כל פרויקט, חוזה והצעת מחיר חדשים ישתמשו בהגדרות שלך אוטומטית!`,
  ...
  },
  {
    ...
    answer: (ctx, gender) => {
      ...
      const specLine = ctx.specialization
        ...
        : "";
      return `${prefix}כרטיס ביקור דיגיטלי — הרושם הראשון שלך! 💳✨\n\n📍 מיקום: תפריט צד ← ראשי ← כרטיס ביקור\n\n🔹 עריכה: שם, התמחות, טלפון, אימייל, אינסטגרם\n🔹 עיצוב: ${g(gender, "בחר", "בחרי")} צבעים ועיצוב שמתאימים לברנד\n🔹 שיתוף: קישור ייחודי + QR code${specLine}\n\n📱 איך משתמשים?\n   📤 ${g(gender, "שלח", "שלחי")} בוואטסאפ ללקוחות פוטנציאליים\n   🖨️ ${g(gender, "הדפס", "הדפיסי")} QR על כרטיסי ביקור פיזיים\n   📸 ${g(gender, "הוסף", "הוסיפי")} לביו באינסטגרם\n\n💡 טיפ: ${g(gender, "שלח", "שלחי")} בסוף כל פגישת ייעוץ — הלקוח שומר ומעביר הלאה!`;
    },
  ...
  },
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `Webhooks — ${g(gender, "חבר", "חברי")} את המערכת לכל דבר! 🔌\n\n📍 מיקום: תפריט צד ← CRM ← Webhooks\n\n🔹 מה זה? התראות אוטומטיות לשירותים חיצוניים — כל מה שקורה במערכת, יכול להפעיל פעולה במקום אחר!\n\n🔹 אירועים נתמכים:\n   👤 לקוח חדש נוצר\n   ✍️ חוזה נחתם\n   ✅ משימה הושלמה\n   💰 הצעה אושרה\n\n🔹 חיבור: ${g(gender, "הגדר", "הגדירי")} URL של השירות החיצוני ← המערכת שולחת אוטומטית\n\n🔥 דוגמאות:\n   📊 Zapier → שורה חדשה ב-Google Sheets\n   📧 Make → מייל אוטומטי ללקוח\n   📱 Slack → התראה לצוות\n\n💡 טיפ: ${g(gender, "התחל", "התחילי")} עם Zapier — קל להגדרה וחינמי ל-100 פעולות!`,
  ...
  },
  {
    ...
    answer: (ctx, gender) => {
      ...
      return `דיווח עסקאות — ${g(gender, "הרווח", "הרוויחי")} מכל עסקה! 🤝${statsLine}\n\n📍 מיקום: תפריט צד ← קהילה ← דיווח עסקה\n\n🔹 תהליך פשוט:\n   1️⃣ ${g(gender, "בחר", "בחרי")} ספק מהרשימה\n   2️⃣ ${g(gender, "הזן", "הזיני")} סכום העסקה\n   3️⃣ ${g(gender, "הוסף", "הוסיפי")} פרטים ← ${g(gender, "שלח", "שלחי")}!\n\n🎁 מה ${g(gender, "אתה מרוויח", "את מרוויחה")}:\n   🎰 כניסות להגרלה — כל עסקה = סיכוי!\n   ⭐ בניית דירוג — ככל ${g(gender, "שמדווח", "שמדווחת")} יותר, הדירוג עולה\n   🤝 חיזוק הקהילה — ספקים טובים מקבלים חשיפה\n\n💡 טיפ: ${g(gender, "דווח", "דווחי")} על כל עסקה, גם קטנה — הכל נספר!`;
    },
  ...
  // --- Portal & Client Management ---
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `כדי לשלוח לינק ללקוח לאזור האישי שלו, ${g(gender, "היכנס", "היכנסי")} ל-CRM > לקוחות > ${g(gender, "לחץ", "לחצי")} על הלקוח > 'שלח לינק לאזור אישי'. הלינק יועתק ${g(gender, "ותוכל", "ותוכלי")} לשלוח אותו בוואצפ. הלקוח ייכנס ישירות בלי צורך בקוד אימות. 🔗\n\n💡 טיפ: ${g(gender, "שלח", "שלחי")} את הלינק ללקוח מיד אחרי שיצרת אותו — ככה הוא יכול למלא פרטים ולראות את ההתקדמות בעצמו!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `${g(gender, "כשאתה יוצר", "כשאת יוצרת")} לקוח חדש, ${g(gender, "אתה יכול", "את יכולה")} למלא שאלון קליטה עם כתובת הנכס, עיר, פרטי שיפוץ, מטרת השיפוץ ותקציב. הלקוח יכול גם למלא את הפרטים בעצמו דרך האזור האישי שלו. 📋\n\n📍 מיקום: CRM ← לקוחות ← לקוח חדש ← שאלון קליטה\n\n💡 טיפ: ככל שהשאלון מלא יותר, כך ${g(gender, "תגיע מוכן", "תגיעי מוכנה")} יותר לפגישה הראשונה!`,
  ...
  // --- Supplier Features ---
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `ספקי הקהילה מסומנים בתג 'ספק קהילה' עם גבול זהב ותג 'מאומת'. ספקים ${g(gender, "שאתה מוסיף", "שאת מוסיפה")} בעצמך יופיעו ללא מיתוג. בבחירת ספק לפרויקט, ספקי הקהילה מופיעים אוטומטית. 🏅\n\n📍 מיקום: תפריט צד ← קהילה ← ספריית ספקים\n\n💡 טיפ: ספקי קהילה עברו אימות ויש להם דירוגים ${g(gender, "ממעצבים אחרים", "ממעצבות אחרות")} — שווה לבדוק אותם קודם!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `${g(gender, "אתה יכול", "את יכולה")} לראות את כל פרטי הספק: שם, טלפון, אימייל, כתובת, איש קשר, אזורי פעילות ותחומי פעילות. דירוגים מוצגים רק ${g(gender, "למנהל", "למנהלת")} הקהילה. 📇\n\n📍 מיקום: ספריית ספקים ← ${g(gender, "לחץ", "לחצי")} על ספק ← כרטיס ספק\n\n💡 טיפ: ${g(gender, "בדוק", "בדקי")} את אזורי הפעילות לפני ${g(gender, "שפונה", "שפונה")} לספק — חוסך זמן!`,
  ...
  // --- Notifications ---
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `${g(gender, "אתה יכול", "את יכולה")} לבחור לקבל עדכונים באימייל או בוואצפ. ${g(gender, "היכנס", "היכנסי")} להגדרות > העדפות התראות ${g(gender, "ובחר", "ובחרי")} את האפשרויות המתאימות לך. 🔔\n\n📍 מיקום: תפריט צד ← הגדרות ← העדפות התראות\n\n🔹 אפשרויות: אימייל 📧 | וואצפ 📱\n🔹 סוגי התראות: חוזה נחתם, משימה חדשה, תזכורת פגישה, ועוד\n\n💡 טיפ: ${g(gender, "הפעל", "הפעילי")} התראות וואצפ — ${g(gender, "תקבל", "תקבלי")} עדכונים בזמן אמת!`,
    relatedTopics: ["הגדרות CRM", "WhatsApp"],
  },
  // --- Business Card (enhanced) ---
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `${g(gender, "אתה יכול", "את יכולה")} ליצור כרטיס ביקור דיגיטלי עם שעות פעילות, תגיות מומחיות, תיק עבודות לפני/אחרי, סטטיסטיקות מקצועיות, QR קוד, וידאו, אנימציות, מצב כהה, כפתור שמירת איש קשר (vCard), וכפתורי ניווט Waze/Maps. 💳✨\n\n📍 מיקום: תפריט צד ← ראשי ← כרטיס ביקור\n\n🔹 פיצ'רים מתקדמים:\n   🕐 שעות פעילות\n   🏷️ תגיות מומחיות\n   📸 תיק עבודות לפני/אחרי\n   📊 סטטיסטיקות מקצועיות\n   📱 QR קוד לשיתוף מהיר\n   🎬 וידאו מקצועי\n   🌙 מצב כהה\n   📇 כפתור שמירת vCard\n   🗺️ כפתורי ניווט Waze/Maps\n\n💡 טיפ: ${g(gender, "הפעל", "הפעילי")} את כל האפשרויות — כרטיס מלא מרשים יותר!`,
  ...
  // --- CRM Features ---
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `ב-CRM ${g(gender, "תוכל", "תוכלי")} לנהל חוזים דיגיטליים ותבניות. ${g(gender, "תוכל", "תוכלי")} להעלות תבנית חוזה ולהוסיף שדות מותאמים. לאחר חתימה, החוזה יופיע גם ברשימת החוזים וגם בכרטיס הלקוח. 📝\n\n📍 מיקום: CRM ← חוזים ← תבניות\n\n🔹 העלאת תבנית: PDF / תמונה\n🔹 שדות מותאמים: שם לקוח, תאריך, סכום, חתימה\n🔹 מעקב: טיוטה → נשלח → נחתם\n\n💡 טיפ: ${g(gender, "צור", "צרי")} תבנית אחת מושלמת ${g(gender, "ותשתמש", "ותשתמשי")} בה שוב ושוב!`,
  ...
  },
  {
    ...
    category: "management",
    answer: (_ctx, gender) => `ב-CRM ${g(gender, "תוכל", "תוכלי")} לנהל לקוחות, ליצור פרויקטים, לעקוב אחרי תקציב, לנהל לוח זמנים, מודבורדים, משימות, ותקשורת עם הלקוח. 💼\n\n📍 מיקום: תפריט צד ← CRM\n\n🔹 כלים עיקריים:\n   👤 לקוחות — ניהול מלא עם 20 כרטיסיות\n   🏗️ פרויקטים — מעקב סטטוס ותקציב\n   📋 משימות — ניהול מטלות לפי עדיפות\n   📅 יומן — פגישות ותזכורות\n   🎨 מודבורד — לוחות השראה\n   💬 WhatsApp — תקשורת ישירה\n\n💡 טיפ: ${g(gender, "התחל", "התחילי")} מהלקוחות — כל שאר הכלים מתחברים דרכם!`,
  ...
  },
  {
    ...
    category: "financial",
    answer: (_ctx, gender) => `ב-CRM > דוחות ${g(gender, "תוכל", "תוכלי")} לראות ניתוח רווחיות, סטטיסטיקות פרויקטים, ועוד. 📊\n\n📍 מיקום: תפריט צד ← CRM ← דוחות\n\n🔹 סוגי דוחות:\n   💰 ניתוח רווחיות — הכנסות מול הוצאות\n   📈 סטטיסטיקות פרויקטים — סטטוסים, משך, תקציב\n   ⏱️ מעקב שעות — סיכום זמן לפי פרויקט\n   👤 דוח לקוחות — מחזור לפי לקוח\n\n💡 טיפ: ${g(gender, "בדוק", "בדקי")} את דוח הרווחיות אחת לחודש — ${g(gender, "תדע", "תדעי")} בדיוק כמה כל פרויקט מרוויח!`,
  ...
  // --- Legal & Terms ---
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `תנאי השימוש באתר זמינים בכתובת /terms. עליך לאשר את תנאי השימוש בכניסה הראשונה. האישור נשמר עם חותמת זמן. 📜\n\n📍 קישור: /terms\n\n🔹 אישור נדרש בכניסה ראשונה בלבד\n🔹 האישור מתועד עם תאריך ושעה\n\n💡 טיפ: ${g(gender, "קרא", "קראי")} את התנאים — הם מגנים גם עליך!`,
  ...
  // --- General ---
  {
    ...
    category: "system",
    answer: (_ctx, gender) => `${g(gender, "אתה יכול", "את יכולה")} להעלות לוגו בפורמט PNG שקוף (מדבקה) כדי שהרקע של האתר יראה מאחורי הלוגו. תומך גם ב-WEBP ו-SVG. 🎨\n\n📍 מיקום: הגדרות ← העלאת לוגו\n\n🔹 פורמטים נתמכים: PNG (שקוף), WEBP, SVG\n🔹 מומלץ: רקע שקוף לתוצאה הכי יפה\n\n💡 טיפ: ${g(gender, "השתמש", "השתמשי")} בלוגו בפורמט PNG שקוף — ככה הוא משתלב יפה בכל רקע!`,
...
function getGreetingMessage(ctx: DesignerContext, gender?: string): string {
  const name = firstName(ctx);
  const timeG = getTimeGreeting();
  const helper = g(gender, "העוזר החכם", "העוזרת החכמה");
  const askMe = g(gender, "שאל אותי כל דבר", "שאלי אותי כל דבר");
  const chooseTopic = g(gender, "בחר נושא", "בחרי נושא");
  const writeQ = g(gender, "כתוב שאלה", "כתבי שאלה");
  const askFree = g(gender, "שאל חופשי", "שאלי חופשי");
  if (name) {
    return pick([
      `${timeG} ${name}! ✨ אני ${helper} של זירת האדריכלות.\n\nאני ${g(gender, "מכיר", "מכירה")} כל פינה במערכת — ${askMe}!\n\n💡 ${chooseTopic} למטה או ${writeQ}.`,
      `היי ${name}! 💛 ${timeG}.\n\nאני כאן לעזור לך עם כל מה שקשור למערכת — מלקוחות וחוזים, דרך עיצוב ותקציב.\n\n💡 ${chooseTopic} או ${askFree}!`,
    ]);
  }
  return `${timeG}! ✨ אני ${helper} של זירת האדריכלות.\n\nאני ${g(gender, "מכיר", "מכירה")} את כל המערכת לעומק — ${askMe}!\n\n💡 ${chooseTopic} למטה או ${writeQ}.`;
}

function getNoMatchMessage(ctx: DesignerContext, gender?: string): string {
  const name = firstName(ctx);
  const prefix = name ? `${name}, ` : "";
  const tryRephrase = g(gender, "נסה לנסח אחרת, או שאל", "נסי לנסח אחרת, או שאלי");
  const writeCmd = g(gender, "כתוב", "כתבי");
  return pick([
    `${prefix}לא הצלחתי למצוא תשובה מדויקת. 🤔\n\n${tryRephrase} על:\n• לקוחות, פרויקטים, חוזים\n• הצעות מחיר, משימות, יומן\n• תקציב, תזמון, השראה\n\nאו ${writeCmd} \"מה אפשר\" לרשימה מלאה!`,
    `${prefix}לא מצאתי מענה מדויק לשאלה. 🤔\n\nאולי ניסוח קצת שונה יעזור? אני ${g(gender, "מתמחה", "מתמחה")} ב:\n📋 ניהול (לקוחות, פרויקטים, משימות)\n💰 כספים (תקציב, הצעות, מעקב שעות)\n🎨 עיצוב (השראה, מודבורד, סגנון)\n\n${writeCmd} \"מה אפשר\" ← רשימה מלאה!`,
  ]);
}

// Enhanced matching algorithm
function findAnswer(input: string, ctx: DesignerContext, gender?: string): { answer: string; relatedTopics: string[] } {
  ...
  for (const pattern of conversationalPatterns) {
    for (const rx of pattern.patterns) {
      if (rx.test(normalized)) {
        const responses = typeof pattern.responses === "function" ? pattern.responses(ctx, gender) : pattern.responses;
  ...
  if (bestMatch && bestScore > 0) {
    const answer = typeof bestMatch.answer === "function"
      ? bestMatch.answer(ctx, gender)
      : bestMatch.answer;
    return { answer, relatedTopics: bestMatch.relatedTopics || [] };
  }

  return { answer: getNoMatchMessage(ctx, gender), relatedTopics: ["מה אפשר לעשות?", "ניווט באתר"] };
}
...
export default function ChatBot({ designerContext, gender }: ChatBotProps) {
  ...
  const handleSend = (text?: string) => {
    ...
    setTimeout(() => {
      const { answer, relatedTopics } = findAnswer(msgText, ctx, gdr);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/30" style={{ fontSize: "10px" }}>
                  {isTyping ? g(gdr, "מקליד...", "מקלידה...") : `${g(gdr, "מחובר", "מחוברת")} • ${g(gdr, "עוזר חכם", "עוזרת חכמה")}`}
                </p>
              </div>
            </div>
          ...
          <div className="p-3 border-t border-white/8" style={{ background: "rgba(255,255,255,0.02)" }}>
            <div className={`flex items-center gap-2 rounded-2xl border transition-all duration-300 px-1 ${sendFlash ? "border-gold/50 shadow-[0_0_12px_rgba(201,168,76,0.15)]" : "border-white/10"}`}>
              <input
                ...
                placeholder={g(gdr, "💬 שאל אותי כל דבר...", "💬 שאלי אותי כל דבר...")}
                disabled={isTyping}
              />
              <button
                onClick={() => handleSend()}
